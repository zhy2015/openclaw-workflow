...
with ThreadPoolExecutor(max_workers=5) as executor:
    executor.map(download_video, results)

print("Downloading finished. Packaging...")
os.system("tar -czf /root/.openclaw/workspace/echoes_of_deep_space_fixed.tar.gz -C /root/.openclaw/workspace " + " ".join([f"scene_{i}.mp4" for i in range(1, 11)]) + " kling_results.txt")
